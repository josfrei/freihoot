...
const JugarPartida = () => {
    ...
    const [nPregunta, setNPregunta] = useState([]) // para insertar en la db
    ...
    useEffect(() => {
        ...
        */
        ...
        socket.on('pasoPreguntaAJugadores', (data) => {
            //    console.log('Recibo en jugador: ', data.idPreg , " y " , data.nPreg);
            setNPregunta(data.nPreg)
    ...
    const responderPregunta = async (idpregunta, nOpcion, valor) => {
        ...
        try {
            await axios.post("http://localhost:4000/respuestas", { pin: id, jugadorNick: nickUsuario, preguntaNumero: nPregunta, puntuacion: puntuacion });
        } catch (error) {
            console.log("Error en jugarPartida enviar respuestas:", error.message);
        }
        document.getElementById("registroRespuesta").innerHTML = "Respuesta registrada"
     ...
     * El tiempo se multiplica por 10
     * La pregrunta acertada se da 200 puntos
