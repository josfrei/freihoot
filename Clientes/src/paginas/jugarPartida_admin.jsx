...
const JugarPartidaAdmin = () => {
    ...
    useEffect(() => {
        ...
        */
        ...
        socket.on('actualizarTemporizador', (data) => {
            document.getElementById("tiempo").innerHTML = data.mensaje
        });
    ...
    const iniciarPartida = () => {
        ...
        /**
         * FALTA CAMBIAR EL ESTADO DE PARTIDA INICIADA Y GESTIONAR EN LE PIN QUE SI ESTA EN INICIADO O NO
         */
        if (preguntas.length > 0) {
            setPreguntaActual(preguntas[0]);
            envíaPregunta(preguntas[0], preguntas[0].tiempo_respuesta, preguntas[0].idPreguntas) /************ probando esto */
            setNPregunta(0);
     ...
     * @param {*} pregunta 
     */
    const envíaPregunta = (pregunta, tiempo, idPreguntas) => {
        // Evío pregunta al servidor
        socket.emit('pasoPreguntaAServidor', { pin: id, pregunta, tiempo, idPreguntas });
    }
    ...
    const siguientePregunta = () => {
        const siguiente = preguntas[nPregunta + 1];
        if (siguiente) {
            envíaPregunta(siguiente, siguiente.tiempo_respuesta, siguiente.idPreguntas) /************ probando esto */
            setNPregunta(nPregunta + 1);
    ...
    return (
        <>
            <div id="contenidoAdmin">
                ...
                {/* si oartida iniciada es false se muestran los jugadores */}
                {!partidaIniciada ? (
                    <>
                        <hr /><hr />
                    ...
                    <>
                        {/* si oartida iniciada es true se muestran las preguntas */}
                        {preguntaActual && (
                            <div>
                                <h1>- {nombreSala} -</h1><br />
                                    ...
                                    {opciones.length > 0 ? (
                                        opciones.map((opcion, index) => (
                                            <div key={index} style={{ display: "inline-block", width: "50%", textAlign: "center" }}>
                                                <button key={index} id="respuesta" style={{ fontSize: "20px", color: colorLetraBoton[index % colorLetraBoton.length], backgroundColor: coloresBotones[index % coloresBotones.length], borderRadius: "55px", width: "200px", height: "200px", padding: "5px", marginTop: "40px", border: "none", cursor: "pointer", margin: "40px" }}>{opcion.texto}</button>
                                            </div>
                                        ))
                                    ) : (
                                ...
                                {/* si hay opciones se pone siguiente, sino se pone el botón de finalizar */}
                                {opciones.length > 0 ? (
